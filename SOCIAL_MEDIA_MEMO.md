...
## 🏷️ 제품 정확한 설명 (사용자 직접 확인 — 이 내용 기준으로 홍보)

### 1. FILO + DINE — 직원 관리자 통합 매장 운영 올인원

> **"FILO와 DINE을 연동해 사장님(FILO) + 직원 관리자(DINE)가 함께 쓰는 통합 시스템"**

- **설치 방식**: 소프트웨어 방식 → 데스크톱 또는 **기존 포스기에 설치** (새 하드웨어 구매 불필요)
- **핵심 기능**: 결제 · 주문 · 직원 근태 · 월급 실시간 정산 · 명세서 자체 웹 + 알림톡 발송
- **특징**: 매장 운영에 필요한 것 전부 하나에 = **올인원 시스템**
- **향후**: FILO 자체 POS 하드웨어 + 소프트웨어 연동 개발 준비 중
...
> **"택배·배달대행의 복잡한 정산을 엑셀 하나로 전부 계산해주는 정산 전문 프로그램"**

- **대상**: 택배 대리점 · 배달대행 업체
- **핵심**: 복잡한 정산 → 엑셀 파일 하나 올리면 모든 게 자동 계산
- **특징**: 딱 맞는 형태로 정산 (과·부족 없이 정확한 정산)
...
> **"주선사업자 없이 기사와 대리점이 직계약으로 연결되는 정보통신망 앱"**

- **법적 근거**: 정보통신망 기반 → 주선사업자 면허 없이 운영 가능
- **매칭 구조**: 대리점이 공고 등록 → 기사가 직접 지원 → 직계약 체결
- **요금**:
  - 기사: 월 150,000원 회원비
  - 대리점: DONWAY 사용 시 용차앱 무료 (DONWAY 구독 연계 혜택)
