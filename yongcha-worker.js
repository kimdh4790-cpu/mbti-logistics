// yongcha-worker.js
// 용차 앱 전용 Cloudflare Worker
// yongcha.app 도메인 전용 — KV 의존 없음
...
function _buildNav(){
  ...
  var tabs;
  if(type==='admin'){
    tabs=[{ico:'home',lbl:'홈',p:'home'},{ico:'list',lbl:'전체공고',p:'posts'},
          {ico:'plus',lbl:'공고등록',p:'post_write'},{ico:'users',lbl:'회원관리',p:'members'},{ico:'user',lbl:'내정보',p:'profile'}];
  } else if(type==='agency'){
    tabs=[{ico:'home',lbl:'홈',p:'home'},{ico:'list',lbl:'내 공고',p:'my_posts'},
          {ico:'plus',lbl:'공고등록',p:'post_write'},{ico:'user',lbl:'내정보',p:'profile'}];
  } else {
    tabs=[{ico:'home',lbl:'홈',p:'home'},{ico:'truck',lbl:'공고',p:'posts'},
          {ico:'pin',lbl:'지원현황',p:'my_applies'},{ico:'user',lbl:'내정보',p:'profile'}];
  }
...
// ── 택배사 CSS 클래스 ────────────────────────────────────────────────────
function _courierClass(c,prefix){
  prefix=prefix||'sc';
  if(!c) return '';
  if(c.indexOf('CJ')>=0) return prefix+'--cj';
  if(c.indexOf('한진')>=0) return prefix+'--hj';
  if(c.indexOf('롯데')>=0) return prefix+'--lt';
  if(c.indexOf('우체국')>=0) return prefix+'--up';
  if(c.indexOf('쿠팡')>=0) return prefix+'--cp';
  if(c.indexOf('로젠')>=0) return prefix+'--rz';
  return '';
}

// ── AI 매칭 스코어 ───────────────────────────────────────────────────────
var _AVG_RATE={'CJ대한통운':880,'한진택배':850,'롯데택배':860,'우체국':900,'쿠팡로지스틱스':950,'로젠택배':840};

function _calcMatchScore(post){
  if(!_CU||_CU.type!=='driver') return null;
  var score=52;
  if(_CU.region&&post.region===_CU.region) score+=28;
  else if(_CU.region&&post.region) score-=8;
  if(_CU.carType&&post.vehicleType&&post.vehicleType.indexOf(_CU.carType)>=0) score+=12;
  var avg=_AVG_RATE[post.courier]||880;
  if(post.unitPrice>avg*1.1) score+=10;
  else if(post.unitPrice<avg*0.9) score-=8;
  if(post.workShift==='야간') score-=5; // 야간 선호 낮음 기본값
  if(post.minGuarantee&&post.minGuarantee>30) score+=6;
  if(post.urgent) score+=4;
  return Math.min(98,Math.max(20,score));
}

function _matchColor(s){
  return s>=80?'#22c55e':s>=60?'#f59e0b':'#7878a0';
}

function _rateVsMarket(price,courier){
  var avg=_AVG_RATE[courier]||880;
  return Math.round((price-avg)/avg*100);
}

function _rateHtml(price,courier){
  var pct=_rateVsMarket(price,courier);
  if(pct>4) return '<span class="sc-rate-badge sc-rate-up">↑시장가 +'+pct+'%</span>';
  if(pct<-4) return '<span class="sc-rate-badge sc-rate-dn">↓시장가 '+pct+'%</span>';
  return '<span class="sc-rate-badge sc-rate-ok">시장 평균</span>';
}

function _timeAgo(ts){
  if(!ts) return '';
  var sec=Math.floor(Date.now()/1000-(ts.seconds||0));
  if(sec<60) return '방금';
...
function _loadPostsRealtime(){
  ...
  _postsUnsub=_db.collection('yongcha_posts').orderBy('createdAt','desc').limit(100).onSnapshot(function(snap){
    ...
    var el=document.getElementById('posts-content');
    if(el) el.innerHTML='<div class="empty"><div class="empty-msg">연결 오류: '+e.message+'</div></div>';
  });
}

// ── RouteIQ 배너 업데이트 ─────────────────────────────────────────────
function _renderRouteIQ(){
  if(_CU.type!=='driver') return;
  var matched=_allPosts.filter(function(p){
    var s=_calcMatchScore(p);
    return s&&s>=70&&p.status==='open';
  });
  var banner=document.getElementById('routeiq-banner');
  if(!banner) return;
  banner.innerHTML=
    '<div class="routeiq-icon">'+_SVG.brain+'</div>'+
    '<div style="flex:1">'+
    '<div class="routeiq-title">RouteIQ AI 분석 완료</div>'+
    '<div class="routeiq-sub">'+_allPosts.length+'개 공고 중 '+matched.length+'개 매칭</div>'+
    '</div>'+
    '<div class="routeiq-badge">'+matched.length+'개 추천</div>';
}
...
function _pgPosts(el){
  ...
  el.innerHTML=
    (isDriver?'<div class="routeiq-banner" id="routeiq-banner">'+
    '<div class="routeiq-icon">'+_SVG.brain+'</div>'+
    '<div style="flex:1"><div class="routeiq-title">RouteIQ 분석 중...</div>'+
    '<div class="ai-analyzing"><span class="ai-dot"></span><span class="ai-dot"></span><span class="ai-dot"></span><span style="margin-left:4px">공고를 분석하고 있어요</span></div></div>'+
    '</div>':'')+

    '<div class="cat-tabs">'+
    '<button class="cat-tab on" id="ctab-yongcha" onclick="_setCatTab(\'yongcha\')">'+
    '<div class="cat-tab-main">용차</div>'+
    '<div class="cat-tab-sub">바로고 · 생각대로 · 배민커넥트</div>'+
    '</button>'+
    '<button class="cat-tab" id="ctab-express" onclick="_setCatTab(\'express\')">'+
    '<div class="cat-tab-main">간선차</div>'+
    '<div class="cat-tab-sub">화물인 · 화물24 · 협력사</div>'+
    '</button>'+
    '</div>'+

    '<div class="region-filter-wrap">'+
    '<div class="region-filter-label">지역</div>'+
    '<div class="filter-row" id="region-filter"></div>'+
    '</div>'+

    '<div id="posts-content"></div>';

  _renderRegionFilter2();
  _loadPostsRealtime();
}

function _setCatTab(t){
  _yongchaTab=t;
  document.getElementById('ctab-yongcha').classList.toggle('on',t==='yongcha');
  document.getElementById('ctab-express').classList.toggle('on',t==='express');
  _renderPostSections();
}

function _renderRegionFilter2(){
  var el=document.getElementById('region-filter');
  if(!el) return;
  el.innerHTML=REGIONS.map(function(r){
    return '<button class="filter-chip'+(r===_activeRegion?' on':'')+'" onclick="_selRegion2(\''+r+'\')">'+r+'</button>';
  }).join('');
}

function _selRegion2(r){
  _activeRegion=r;
  _renderRegionFilter2();
  _renderPostSections();
}
...
function _renderPostSections(){
  ...
  if(_yongchaTab==='express'){
    _renderExpressTab(el);
    return;
  }

  // 필터
  var posts=_allPosts.filter(function(p){
    if(_activeRegion!=='전체'&&p.region!==_activeRegion) return false;
    return true;
  });

  var open=posts.filter(function(p){return p.status==='open';});
  var urgent=open.filter(function(p){return p.urgent;});
  var matched=_CU.type==='driver'
    ? open.filter(function(p){var s=_calcMatchScore(p);return s&&s>=70;}).sort(function(a,b){return _calcMatchScore(b)-_calcMatchScore(a);})
    : [];
  ...
  // 긴급 공고
  if(urgent.length){
    html+=_slideSection('긴급 공고',urgent,'urgent',true);
  }

  // AI 추천 (기사만)
  if(_CU.type==='driver'&&matched.length){
    html+=_slideSection('AI 추천 공고',matched,'ai',true);
  }

  // 전체/최신 공고
  html+=_slideSection('최신 공고',recent,'recent',true);

  el.innerHTML=html||'<div class="empty"><div class="empty-ico">'+_SVG.search+'</div><div class="empty-msg">해당 조건의 공고가 없어요</div></div>';

  // 더보기 버튼 클릭 이벤트 (after innerHTML)
}

function _slideSection(title,posts,key,live){
  if(!posts.length) return '';
  var MAX=10;
  var shown=posts.slice(0,MAX);
  var cards=shown.map(function(d){return _makeSlideCard(d);}).join('');
  return '<div class="slide-section">'+
    '<div class="slide-hdr" style="padding:0 0 0 0">'+
    (live?'<span class="live-dot"></span>':'')+
    '<span class="slide-hdr-title">'+title+'</span>'+
    '<span class="slide-hdr-count">'+posts.length+'건</span>'+
    '</div>'+
    '<div class="slide-track">'+cards+'</div>'+
    '</div>';
}

// ── 슬라이드 카드 ────────────────────────────────────────────────────────
function _makeSlideCard(d){
  ...
  var statusBadge=isClosed?'<span class="badge badge-closed">마감</span>':
    isMatched?'<span class="badge badge-matched">매칭완료</span>':
    d.urgent?'<span class="badge badge-urgent">긴급</span>':
    '<span class="badge badge-open">모집중</span>';

  var shiftBadge=d.workShift==='야간'?'<span class="badge badge-night">야간</span>':
    d.workShift==='주간'?'<span class="badge badge-day">주간</span>':'';

  var tags=[];
  if(d.vehicleType) tags.push(d.vehicleType);
  if(d.areaAptRatio) tags.push('아파트'+d.areaAptRatio+'%');
  if(d.volume) tags.push(d.volume+'건');

  var scoreHtml=score?
    '<div class="sc-ai"><div class="sc-ai-score" style="color:'+_matchColor(score)+'">'+score+'%</div><div class="sc-ai-label">AI 매칭</div></div>':
    '';

  var card='<div class="sc '+_courierClass(d.courier)+' '+(isClosed?'closed':'')+'" onclick="'+(isClosed?'':'_showPostDetail('+JSON.stringify(d)+')')+'">'+
    '<div class="sc-row1">'+
    '<span class="sc-courier">'+d.courier+'</span>'+
    '<div class="sc-badges">'+shiftBadge+statusBadge+'</div>'+
    '</div>'+
    '<div>'+
    '<div class="sc-area">'+d.region+' '+d.area+'</div>'+
    (tags.length?'<div class="sc-tags">'+tags.map(function(t){return '<span class="sc-tag">'+t+'</span>';}).join('')+'</div>':'')+
    '</div>'+
    '<div class="sc-foot">'+
    '<div>'+
    '<div><span class="sc-price">'+Number(d.unitPrice||0).toLocaleString()+'</span><span class="sc-price-unit">원/건</span></div>'+
    (dayEst?'<div class="sc-est">예상 '+dayEst+'만원/일</div>':'')+
    '<div style="margin-top:4px">'+_rateHtml(d.unitPrice||0,d.courier)+'</div>'+
    '</div>'+
    scoreHtml+
    '</div>'+
    '<div class="sc-meta">'+
    '<span class="sc-agency">'+d.agencyName+'</span>'+
    '<span class="sc-time">'+_timeAgo(d.createdAt)+'</span>'+
    '</div>'+
    '</div>';

  // wrap in div to avoid onclick conflict
  var wrap=document.createElement('div');
  wrap.innerHTML=card;
  var el=wrap.firstChild;
  if(!isClosed) el.onclick=function(){_showPostDetail(d);};
  return el.outerHTML;
}

// ── 간선차 탭 ────────────────────────────────────────────────────────────
function _renderExpressTab(el){
  el.innerHTML=
    '<div class="express-banner">'+
    '<div class="express-banner-icon">'+_SVG.truck+'</div>'+
    '<div class="express-banner-title">전국 간선 노선 매칭</div>'+
    '<div class="express-banner-sub">화물인, 화물24 등 주요 플랫폼의<br>간선 화물차 노선을 한 곳에서 확인하세요</div>'+
    '<div class="express-partners">'+
    '<span class="express-partner">화물인</span>'+
    '<span class="express-partner">화물24</span>'+
    '<span class="express-partner">협력사직접</span>'+
    '<span class="express-partner">로지스</span>'+
    '</div>'+
    '</div>'+
    '<div id="express-list-wrap">'+
    '<div style="text-align:center;padding:20px"><div class="spinner"></div></div>'+
    '</div>';

  // 간선차는 yongcha_posts에서 jobType==='express' OR 별도 컬렉션
  ...
  .then(function(snap){
    ...
    var wrap=document.getElementById('express-list-wrap');
    if(!wrap) return;
    if(!posts.length){
      wrap.innerHTML=
        '<div class="slide-section">'+
        '<div class="slide-hdr"><span class="live-dot"></span><span class="slide-hdr-title">파트너사 공고 연동 준비 중</span></div>'+
        '<div style="background:var(--bg2);border:1px solid var(--bd);border-radius:var(--r);padding:20px;text-align:center">'+
        '<div style="font-size:13px;color:var(--t2);line-height:1.8">화물인·화물24 API 연동 중<br><span style="color:var(--ac);font-weight:700">빠른 시일 내 오픈 예정</span></div>'+
        '<button onclick="_goPage(\'posts\');_setCatTab(\'yongcha\')" class="btn-main" style="margin-top:16px;padding:10px">용차 공고 보기</button>'+
        '</div></div>';
      return;
    }
    wrap.innerHTML=_slideSection('간선 노선 공고',posts,'express',true);
  }).catch(function(){
    var wrap=document.getElementById('express-list-wrap');
    if(wrap) wrap.innerHTML='<div class="empty"><div class="empty-msg">공고를 불러올 수 없어요</div></div>';
  });
}

// ── _makePostCard (레거시 호환 + 새 디자인) ───────────────────────────────
function _makePostCard(d,mini){
  var el=document.createElement('div');
  el.innerHTML=_makeSlideCard(d);
  var card=el.firstChild;
  // vertical list용 - width 제한 해제
  if(card){
...
// ── 필터 호환 함수들 ─────────────────────────────────────────────────────
function _renderCourierTabs(){} // deprecated - no-op
function _selectCourier(c){_activeCourier=c;}
function _renderCourierFilter(){}
function _renderRegionFilter(){_renderRegionFilter2();}
function _selCourier(c){_activeCourier=c;}
function _selRegion(r){_selRegion2(r);}
...
export default {
  async fetch(request, env, ctx) {
      ...
      try {
        ...
        if (body.token && env.FCM_SERVER_KEY) {
          await fetch('https://fcm.googleapis.com/fcm/send', {
            ...
            body: JSON.stringify({
              to: body.token,
              notification: { title: body.title || '용차', body: body.body || '' },
              data: body.data || {}
            })
          });
        }
    ...
    // 모든 경로 → yongcha.html 직접 서빙 (KV 없음!)
