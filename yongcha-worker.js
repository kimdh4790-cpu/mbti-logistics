...
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link rel="preconnect" href="https://cdn.jsdelivr.net"><link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/static/pretendard.min.css">
...
function _buildNav(){
  ...
  if(type==='admin'){
    ...
    tabs=[{ico:'home',lbl:'홈',p:'home'},{ico:'search',lbl:'노선공고',p:'posts'},
...
// ── 택배사 키 → CSS 클래스 ──────────────────────────────────────────────
function _courierClass(courier){
  if(!courier) return '';
  if(courier.indexOf('CJ')>=0) return 'post-card--cj';
  if(courier.indexOf('한진')>=0) return 'post-card--hj';
  if(courier.indexOf('롯데')>=0) return 'post-card--lt';
  if(courier.indexOf('우체국')>=0) return 'post-card--up';
  if(courier.indexOf('쿠팡')>=0) return 'post-card--cp';
  if(courier.indexOf('로젠')>=0) return 'post-card--rz';
  return '';
}
...
function _makePostCard(d,mini){
  ...
  if(d.workDays) tags.push(d.workDays);

  card.innerHTML=
    '<div class="post-row1">'+
    '<span class="post-courier">'+d.courier+'</span>'+
    '<div class="post-badges">'+shiftBadge+statusBadge+'</div>'+
    '</div>'+
    '<div class="post-area">'+d.region+' '+d.area+'</div>'+
    (tags.length?'<div class="post-tags">'+tags.map(function(t){return '<span class="post-tag">'+t+'</span>';}).join('')+'</div>':'')+
    '<div class="post-foot">'+
    '<div><span class="post-price-main">'+Number(d.unitPrice||0).toLocaleString()+'</span><span class="post-price-unit">원/건</span>'+
    (dayEst>0?'<div class="post-est">예상 '+dayEst+'만원/일 ('+d.volume+'건)</div>':'')+'</div>'+
    '<div style="text-align:right"><span class="post-guarantee">일 최소 '+minGuarantee+'만원</span>'+
    '<div class="post-agency">'+d.agencyName+'</div></div>'+
    '</div>';
...
function _pgPosts(el){
  el.innerHTML=
    '<div class="page-hdr"><div class="page-title">노선 공고</div></div>'+
    '<div class="post-tabs">'+
    '<button class="post-tab on" id="ptab-all" onclick="_setPostTab(\'all\')">전체공고</button>'+
    '<button class="post-tab" id="ptab-match" onclick="_setPostTab(\'match\')">맞춤공고</button>'+
    '</div>'+
    '<div class="filter-section">'+
    '<div class="filter-label">택배사</div>'+
    '<div class="filter-row" id="courier-filter"></div>'+
    '</div>'+
    '<div class="filter-section">'+
  ...
  _renderCourierFilter();
  _renderRegionFilter();
  _loadPosts();
}

function _setPostTab(t){
  _postTab=t;_postPage=1;
  document.getElementById('ptab-all').classList.toggle('on',t==='all');
  document.getElementById('ptab-match').classList.toggle('on',t==='match');
  _renderPostList();
}

function _renderCourierFilter(){
  var el=document.getElementById('courier-filter');
  if(!el) return;
  el.innerHTML=COURIERS.map(function(c){
    return '<button class="filter-chip'+(c===_activeCourier?' on':'')+'" onclick="_selCourier(\''+c+'\')">'+c+'</button>';
  }).join('');
}
function _renderRegionFilter(){
  ...
  el.innerHTML=REGIONS.map(function(r){
    return '<button class="filter-chip'+(r===_activeRegion?' on':'')+'" onclick="_selRegion(\''+r+'\')">'+r+'</button>';
  }).join('');
}
function _selCourier(c){_activeCourier=c;_postPage=1;_renderCourierFilter();_renderPostList();}
function _selRegion(r){_activeRegion=r;_postPage=1;_renderRegionFilter();_renderPostList();}
// keep old alias for compat
function _renderCourierTabs(){_renderCourierFilter();}
function _selectCourier(c){_selCourier(c);}
...
function _loadPosts(){
  ...
  _db.collection('yongcha_posts').orderBy('createdAt','desc').limit(200).get()
  .then(function(snap){
    _allPosts=[];
...
function _renderPostList(){
  var listEl=document.getElementById('posts-list');
  var pagEl=document.getElementById('posts-pagination');
  if(!listEl) return;

  var filtered=_allPosts.filter(function(p){
    if(_activeCourier!=='전체'&&p.courier!==_activeCourier) return false;
    if(_activeRegion!=='전체'&&p.region!==_activeRegion) return false;
    if(_postTab==='match'&&_CU.region&&p.region!==_CU.region) return false;
    return true;
  });

  // 긴급/open 우선
  filtered.sort(function(a,b){
    var ao=a.status==='open'||a.status==='matched';
    var bo=b.status==='open'||b.status==='matched';
  ...
  if(_postPage>pages) _postPage=pages;
  var slice=filtered.slice((_postPage-1)*PER,_postPage*PER);

  if(!total){
    listEl.innerHTML='<div class="empty"><div class="empty-ico">'+_SVG.search+'</div><div class="empty-msg">해당 조건의 공고가 없어요</div></div>';
    if(pagEl) pagEl.innerHTML='';
    return;
  }

  listEl.innerHTML='';
  slice.forEach(function(d){listEl.appendChild(_makePostCard(d,false));});

  // 페이지네이션
  if(pagEl){
    var html='';
    html+='<button class="page-btn" onclick="_changePage('+(_postPage-1)+')" '+((_postPage<=1)?'disabled':'')+'>'+
      '<svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg></button>';
    var start=Math.max(1,_postPage-2),end=Math.min(pages,start+4);
    for(var i=start;i<=end;i++){
      html+='<button class="page-btn'+(i===_postPage?' on':'')+'" onclick="_changePage('+i+')">'+i+'</button>';
    }
    html+='<button class="page-btn" onclick="_changePage('+(_postPage+1)+')" '+((_postPage>=pages)?'disabled':'')+'>'+
