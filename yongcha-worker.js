...
function _showLogin(){
  ...
  if(lBtn){lBtn.textContent='로그인';lBtn.disabled=false;}
  var rBtn=document.getElementById('r-btn');
  if(rBtn){rBtn.textContent='가입하기';rBtn.disabled=false;}
  document.getElementById('login-screen').style.display='flex';
...
// HTML 이스케이프 — innerHTML 로 들어가는 모든 사용자 입력에 적용
function _esc(s){
  if(s===null||s===undefined)return '';
  return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')
    .replace(/"/g,'&quot;').replace(/'/g,'&#39;');
}
// onclick="fn('...')" 안에 들어가는 문자열 — 따옴표/역슬래시/개행 이스케이프
...
function _ago(ts){
  ...
  return new Date(ts.seconds*1000).toLocaleDateString('ko-KR',{month:'numeric',day:'numeric'});
}
function _won(n){return Number(n||0).toLocaleString('ko-KR');}

/* ── 카운트업 애니메이션 ── */
function _yCountUp(el,from,to,ms,suffix){
  if(!el)return;
  var start=null,range=to-from;
  if(range===0){el.textContent=_won(to)+(suffix||'');return;}
  function step(ts){
    if(!start)start=ts;
    var p=Math.min((ts-start)/ms,1);
    var ease=1-Math.pow(1-p,4); // ease-out-quart
    var cur=Math.round(from+range*ease);
    el.textContent=_won(cur)+(suffix||'');
...
/* ── 스켈레톤 로딩 ── */
function _sk(w,h,r,mb){
  return '<div class="skel" style="width:'+(w||'100%')+';height:'+(h||12)+'px'+
    (r?';border-radius:'+r:'')+(mb!=null?';margin-bottom:'+mb+'px':'')+'"></div>';
}
// 공고 카드 스켈레톤 — 실제 카드와 같은 골격
function _skeletonCards(n){
  var out='';
  for(var i=0;i<(n||3);i++){
    out+='<div class="skel-card">'+
      '<div class="skel-row">'+_sk('96px',26,'999px')+_sk('64px',26,'999px')+'</div>'+
      _sk('72%',19,null,9)+_sk('44%',12,null,16)+
      _sk('52%',29,null,14)+
      '<div style="display:flex;gap:6px;margin-bottom:14px">'+
        _sk('58px',28,'8px')+_sk('86px',28,'8px')+_sk('66px',28,'8px')+
      '</div>'+
      _sk('100%',42,'12px')+
    '</div>';
  }
...
function _skRows(n){
  var out='';
  for(var i=0;i<(n||4);i++){
    out+='<div class="skel-card" style="display:flex;align-items:center;gap:12px;padding:15px;margin-bottom:10px">'+
      _sk('44px',44,'14px')+
      '<div style="flex:1">'+_sk('58%',14,null,8)+_sk('36%',11,null,0)+'</div>'+
      _sk('56px',24,'999px')+
    '</div>';
  }
  return out;
}
var _skVal='<span class="skel" style="display:inline-block;width:58px;height:26px;vertical-align:-5px"></span>';

function _emptyHtml(ico,msg,sub){
  return '<div class="empty"><div class="empty-ico">'+ico+'</div>'+
    '<div class="empty-msg">'+msg+'</div>'+
    (sub?'<div class="empty-sub">'+sub+'</div>':'')+'</div>';
}
function _errHtml(e){
  return _emptyHtml('!','불러오지 못했어요',_esc((e&&e.message)||'네트워크를 확인해주세요'));
}
...
/* ══ 차종 — 하이탑 / 로우탑 두 가지로 통일 ══════════════════════
   과거 데이터에 '1톤 하이탑' 처럼 톤수가 붙은 값이 섞여 있어
   그대로 비교하면 매칭이 전부 실패한다 → 항상 _yCarNorm 으로 정규화해서 비교. */
var CAR_TYPES=['하이탑','로우탑'];
function _yCarNorm(v){
  var s=String(v==null?'':v).trim();
  if(!s)return '';
  if(s.indexOf('하이탑')!==-1)return '하이탑';
  if(s.indexOf('로우탑')!==-1)return '로우탑';
  if(s==='무관')return '';
  return s;
}
function _yCarOptions(sel,includeAny){
  var list=includeAny?CAR_TYPES.concat(['무관']):CAR_TYPES;
  return '<option value="">차종 선택</option>'+list.map(function(v){
    return '<option value="'+v+'"'+(_yCarNorm(sel)===_yCarNorm(v)?' selected':'')+'>'+v+'</option>';
  }).join('');
}
...
function _yLogin(){
  ...
  _auth.signInWithEmailAndPassword(e,p).catch(function(ex){
    ...
    err.textContent=c==='auth/wrong-password'||c==='auth/invalid-credential'?'비밀번호가 틀렸어요':
                    c==='auth/user-not-found'?'없는 계정이에요':
                    c==='auth/too-many-requests'?'잠시 후 다시 시도하세요':
                    c==='auth/invalid-email'?'이메일 형식을 확인하세요':
                    '오류: '+c;
...
function _showApp(){
  ...
  _prefs=_CU.preferences||null;
  _buildNav();
  // 찜 목록을 먼저 캐시한 뒤 홈을 그린다 (카드마다 개별 read 하지 않기 위해)
  _yLoadBookmarks().then(function(){
    return _yRestSync();          // 만료된 휴식 자동 복귀 처리 후 홈을 그린다
  }).then(function(){
    _goPage('home');
    _yHandleDeepLink();
  });
...
// ══ 하단 5탭 고정 내비 ═══════════════════════════════════════
// 역할별 항상 정확히 5탭 (레이아웃 안정성 + 근육기억)
// 요구 사양: 홈 / 공고 / 채팅 / 내노선 / 프로필 — 5탭 고정, 역할별로 대상만 다르게 매핑.
// 나머지(구인구직·휴식원장·월대시보드·관리)는 홈 퀵액션에서 1탭 → 총 2단계로 접근.
/* ── Lucide 스타일 SVG 아이콘 ── */
var _ico={
  ...
  truck:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>',
  pkg:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M16.5 9.4l-9-5.19M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>',
  wallet:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12V7H5a2 2 0 0 1 0-4h14v4"/><path d="M3 5v14a2 2 0 0 0 2 2h16v-5"/><path d="M18 12a2 2 0 0 0 0 4h4v-4z"/></svg>',
  user:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',
  list:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>',
  users:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>',
  more:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg>',
  ...
  dash:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>'
};
var NAV_TABS={
  admin:[{ico:_ico.home,lbl:'홈',p:'home'},{ico:_ico.list,lbl:'공고',p:'posts'},
         {ico:_ico.chat,lbl:'채팅',p:'chat'},{ico:_ico.users,lbl:'회원관리',p:'members'},
         {ico:_ico.user,lbl:'프로필',p:'profile'}],
...
function _buildNav(){
  var tabs=NAV_TABS[_CU.type]||NAV_TABS.driver;
  document.getElementById('bnav').innerHTML=tabs.map(function(t){
    return '<button type="button" class="bnav-btn" id="bnav-'+t.p+'" aria-label="'+t.lbl+'" onclick="_goPage(\\''+t.p+'\\')">'+
      '<span class="bnav-ico">'+t.ico+'</span>'+
      '<span class="bnav-lbl">'+t.lbl+'</span>'+
      (t.p==='chat'?'<span class="nav-badge" id="nav-badge-chat" style="display:none">0</span>':'')+
      '</button>';
  }).join('');
  _yStartChatBadge();
}
...
function _goPage(p){
  ...
  var nb=document.getElementById('bnav-'+p);if(nb)nb.classList.add('on');
  if(p==='home')_pgHome(el);
  else if(p==='posts')_pgPosts(el);
  else if(p==='routeiq')_pgRouteIQ(el);
  else if(p==='my_posts')_pgMyPosts(el);
  else if(p==='post_write'){_pgPostWrite(el);}
  else if(p==='my_applies')_pgMyApplies(el);
  else if(p==='my_routes')_pgMyRoutes(el);
  else if(p==='jobs')_pgJobs(el);
  else if(p==='profile')_pgProfile(el);
  else if(p==='members')_pgMembers(el);
  else if(p==='chat')_pgChats(el);
...
// ── 채팅 안읽음 배지 (실시간) ──
var _chatBadgeUnsub=null;
function _yStartChatBadge(){
  if(_chatBadgeUnsub){_chatBadgeUnsub();_chatBadgeUnsub=null;}
  if(!_CU)return;
  _chatBadgeUnsub=_db.collection('yongcha_chats').where('participants','array-contains',_CU.uid)
    .onSnapshot(function(snap){
      var total=0;
      snap.forEach(function(d){total+=((d.data().unread||{})[_CU.uid]||0);});
      var b=document.getElementById('nav-badge-chat');
      if(b){b.textContent=total>99?'99+':total;b.style.display=total>0?'block':'none';}
    },function(){});
}

// ── 홈 실시간 리스너 관리 (관리자 KPI) ──
var _homeUnsubs=[];
function _yStopHomeWatch(){
...
function _pgHome(el){
  var type=_CU.type;
  if(type==='driver') return _pgHomeDriver(el);
  if(type==='agency') return _pgHomeAgency(el);
  // 관리자
  el.innerHTML=
  '<div class="hero-card hero-card-admin">'+
  '<div class="hero-tag" style="color:var(--pu)">관리자</div>'+
  '<div class="hero-name">'+_esc(_CU.name)+'님 안녕하세요!</div>'+
  '<div class="hero-region">'+_esc(_CU.region||'—')+'</div>'+
  '</div>'+
  '<div class="kpi-grid col2" id="home-kpi">'+
  '<div class="kpi-card"><div class="kpi-val" style="color:var(--br)" id="kpi1">'+_skVal+'</div><div class="kpi-lbl">전체 공고</div></div>'+
  '<div class="kpi-card"><div class="kpi-val" style="color:var(--gn)" id="kpi2">'+_skVal+'</div><div class="kpi-lbl">전체 회원</div></div>'+
  '<div class="kpi-card"><div class="kpi-val" style="color:var(--pu)" id="kpi3">'+_skVal+'</div><div class="kpi-lbl">프리미엄</div></div>'+
  '<div class="kpi-card"><div class="kpi-val" style="color:var(--rd)" id="kpi4">'+_skVal+'</div><div class="kpi-lbl">정지 회원</div></div>'+
  '</div>'+
  '<div class="sec-head"><span class="sec-title">최신 공고</span></div>'+
  '<div id="home-recent">'+_skeletonCards(2)+'</div>';
  _homeUnsubs.push(_db.collection('yongcha_posts').where('status','==','open').onSnapshot(function(s){
    var e=document.getElementById('kpi1');if(e)e.textContent=s.size+'건';
  },function(){}));
  _homeUnsubs.push(_db.collection('yongcha_users').onSnapshot(function(s){
    var e=document.getElementById('kpi2');if(e)e.textContent=s.size+'명';
    var suspended=0;s.forEach(function(d){if(d.data().suspended)suspended++;});
    var e4=document.getElementById('kpi4');if(e4)e4.textContent=suspended+'명';
  },function(){}));
  _homeUnsubs.push(_db.collection('yongcha_posts').where('premium','==',true).onSnapshot(function(s){
    var e=document.getElementById('kpi3');if(e)e.textContent=s.size+'건';
  },function(){}));
  _db.collection('yongcha_posts').where('status','==','open').limit(5).get().then(function(snap){
...
function _pgHomeDriver(el){
  el.innerHTML=
  // 히어로 카드 — 파란 배경 + 대형 수익 (모형 일치)
  '<div class="hero-v2">'+
    '<div class="hero-v2-top">'+
      '<span class="hero-v2-name">'+_esc(_CU.name)+'님 안녕하세요!</span>'+
      '<span class="hero-v2-badge" id="drv-active-badge">배차 확인 중...</span>'+
    '</div>'+
  ...
  '<div class="quick-grid">'+
    '<button type="button" class="quick-btn" onclick="_goPage(\\'dashboard\\')">'+
      '<span><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg></span>월 대시보드</button>'+
  ...
  _db.collection('yongcha_daily_records')
    .where('driverId','==',_CU.uid).where('date','==',_yToday()).get()
  .then(function(s){
    var total=0,cnt=0;
    s.forEach(function(d){var r=d.data();total+=r.amount||0;cnt+=r.count||0;});
    var earn=document.getElementById('earn-today');
    var sub=document.getElementById('earn-sub-lbl');
    var chips=document.getElementById('earn-chips');
    if(earn){
      if(total>0){
        _yCountUp(earn,0,total,900,'원');
      } else {
        earn.textContent='—';
      }
    }
    if(sub)sub.textContent=total>0?'오늘 '+cnt+'건 완료 · '+_esc(_CU.region||'')+'':'아직 오늘 운행 기록 없어요';
    if(chips&&total>0){
      chips.innerHTML=
        '<span class="hero-v2-chip">'+cnt+'건 완료</span>'+
  ...
  .then(function(snap){
    ...
    snap.docs.slice(0,3).forEach(function(doc,i){
      ...
      row.innerHTML='<div class="sched-num">'+(i+1)+'</div>'+
        '<div><div class="sched-title">'+_esc(a.postArea||a.postRegion||'—')+'</div>'+
...
function _pgHomeAgency(el){
  var skStat='<div class="stat3-val">'+_skVal+'</div>';
  el.innerHTML=
  // 소장 헤더
  '<div class="agency-v2-hdr">'+
    '<div class="agency-v2-name">'+_esc(_CU.name)+'</div>'+
    '<div class="agency-v2-sub">안녕하세요, 소장님! · '+_esc(_CU.region||'지역 미설정')+'</div>'+
  '</div>'+
  ...
  // 최근 지원자
  '<div class="sec-head" style="margin-top:6px"><span class="sec-title">최근 지원자</span><button type="button" class="sec-count" onclick="_goPage(\\'my_routes\\')">전체보기 ›</button></div>'+
  ...
  .then(function(snap){
    var rdl=document.getElementById('region-dispatch');if(!rdl)return;
    var regionMap={};
    snap.forEach(function(doc){
      var r=doc.data().driverRegion||'미지정';
      regionMap[r]=(regionMap[r]||0)+1;
    });
    var regions=Object.keys(regionMap).sort(function(a,b){return regionMap[b]-regionMap[a];}).slice(0,5);
    if(!regions.length){rdl.innerHTML='<div style="padding:16px 14px;font-size:13px;color:var(--t3);text-align:center">배차 데이터가 없어요</div>';return;}
    var maxVal=regionMap[regions[0]]||1;
    rdl.innerHTML='';
    regions.forEach(function(r){
      var cnt=regionMap[r];
      var row=document.createElement('div');row.className='rdl-row';
      row.innerHTML='<span class="rdl-region">'+_esc(r)+'</span>'+
        '<div class="rdl-bar-wrap"><div class="rdl-bar" style="width:'+Math.round(cnt/maxVal*100)+'%"></div></div>'+
  ...
  .then(function(snap){
    var recent=document.getElementById('home-recent');if(!recent)return;
    if(snap.empty){recent.innerHTML=_emptyHtml('','새 지원자가 없어요','공고를 등록하면 지원자를 받을 수 있어요');return;}
    recent.innerHTML='';
    snap.docs.slice(0,5).forEach(function(doc){
      ...
      card.innerHTML=
        '<div class="applicant-top"><span class="applicant-name">'+_esc(a.driverName)+'</span>'+
        '<span class="st st-open">검토 대기</span></div>'+
  ...
  if(isDriver){
    _db.collection('yongcha_applies')
      ...
      .then(function(snap){
        ...
        if(!snap.empty&&d.status==='open'){
          var sw=document.getElementById('slide-wrap');
          var st=document.getElementById('slide-track');
          if(sw)sw.style.opacity='.5';
          if(st)st.textContent='이미 지원한 공고예요';
        }
      });
    ...
    if(d.status==='open'){
      ...
      var _cdInterval=setInterval(function(){
        ...
        if(numEl)numEl.textContent=_cdSec;
        if(ring)ring.setAttribute('stroke-dashoffset',String(Math.round(144.5*(_cdSec)/5)));
        if(_cdSec<=0){clearInterval(_cdInterval);}
      },1000);
      ...
      setTimeout(function(){
        ...
        var maxX=wrap.offsetWidth-54;
        var dragging=false,startX=0;
        var getX=function(e){return e.touches?e.touches[0].clientX:e.clientX;};
        var onStart=function(e){
          dragging=true;startX=getX(e)-(parseInt(thumb.style.left)||4);
          thumb.style.transition='none';e.preventDefault();
        };
        var onMove=function(e){
          if(!dragging)return;
          e.preventDefault();
          var x=Math.max(4,Math.min(getX(e)-startX,maxX));
          thumb.style.left=x+'px';
          if(x>=maxX-2){
            dragging=false;
            thumb.classList.add('done');thumb.style.left=maxX+'px';
            var track=document.getElementById('slide-track');
            if(track)track.textContent='지원 완료!';
            clearInterval(_cdInterval);
            setTimeout(function(){_applyPost(d.id,d.agencyId,_jsq(d.agencyName));},250);
          }
        };
        var onEnd=function(){
          if(!dragging)return;dragging=false;
          var cur=parseInt(thumb.style.left)||4;
          if(cur<maxX-2){thumb.style.transition='left .22s var(--ease)';thumb.style.left='4px';}
        };
...
/* 지원하기.
   이전 버전은 postId 만 저장하고 노선 조건(courier/area/unitPrice/startDate/
   workShift/workDays/settleDay/minGuarantee)을 저장하지 않았다. 그 결과
...
function _applyPost(postId,agencyId,agencyName,btnEl){
  ...
  getPost.then(function(p){
    ...
    // 휴식 상태를 서버 값으로 재확인 (다른 기기에서 승인됐을 수 있다)
    return _db.collection('yongcha_users').doc(_CU.uid).get().then(function(us){
      var u=us.exists?us.data():{};
      _CU.resting=!!u.resting;_CU.restUntil=u.restUntil||'';_CU.restId=u.restId||'';
      if(_yIsResting()){
        fail(' 휴식 중에는 지원할 수 없어요'+(_CU.restUntil?' ('+_CU.restUntil+'까지)':''));
        return null;
      }
      return p;
    });
    ...
    if(p===null)return null;
    // 중복 지원 방지 (UI 뿐 아니라 실제로 막는다)
    return _db.collection('yongcha_applies')
      .where('postId','==',postId).where('driverId','==',_CU.uid).limit(1).get()
      .then(function(dup){
        if(!dup.empty){fail('이미 지원한 공고예요');return null;}
        p=p||{};
        return _db.collection('yongcha_applies').add({
          ...
          // ── 노선 스냅샷 (계약서·정산·내노선이 참조) ──
          courier:p.courier||'', area:p.area||'', routeNo:p.routeNo||'',
          region:p.region||'', unitPrice:p.unitPrice||0, priceType:p.priceType||'건당',
          volume:p.volume||0, workShift:p.workShift||'', workDays:p.workDays||'',
          workHours:p.workHours||'', startDate:p.startDate||'',
          settleFreq:p.settleFreq||'', settleDay:p.settleDay||'',
          minGuarantee:p.minGuarantee||0, vatIncluded:p.vatIncluded||'',
          status:'pending',
    ...
    if(!ref)return;
    // 지원자 수 카운터 (카드에 표시되지만 지금까지 증가하지 않았음)
    _db.collection('yongcha_posts').doc(postId).update({
...
/* 지원 취소 — 대리점 검토 전(pending)에만 가능 */
function _yCancelApply(applyId,postId){
  if(!confirm('지원을 취소할까요?'))return;
  _db.collection('yongcha_applies').doc(applyId).delete().then(function(){
    if(postId){
      _db.collection('yongcha_posts').doc(postId).update({
        applicantCount:firebase.firestore.FieldValue.increment(-1)
      }).catch(function(){});
...
function _pgMyPosts(el){
  ...
  _db.collection('yongcha_posts').where('agencyId','==',_CU.uid).get()
    .then(function(snap){
      ...
      var _mpAll=snap.docs.sort(function(a,b){var at=a.data().createdAt;var bt=b.data().createdAt;return (bt&&bt.seconds||0)-(at&&at.seconds||0);})
        .map(function(doc){return Object.assign({id:doc.id},doc.data());});
      // 내 공고 목록도 중복 접기 (지난 데이터에 쌓인 중복 대응)
      var _mdd=_yDedupe(_mpAll,_yPostKey);
      list.innerHTML=_yDupNotice(_mdd.hidden,'공고')+
        (_mdd.hidden?'<button onclick="_yPurgeMyDupPosts()" style="width:100%;min-height:44px;margin-bottom:12px;background:var(--rdl);color:var(--rd);border:1px solid var(--rdln);border-radius:var(--r);font-size:13px;font-weight:800;cursor:pointer;font-family:inherit">🗑 중복 '+_mdd.hidden+'건 영구 삭제</button>':'');
      _mdd.items.forEach(function(d){
        var card=document.createElement('div');card.className='card';
        card.innerHTML=
          '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">'+
          '<div style="font-size:16px;font-weight:800;letter-spacing:-.4px">'+_esc((d.region||'')+' '+(d.area||''))+'</div>'+
          '<span class="status-badge '+(d.status==='open'?'badge-open':d.status==='matched'?'badge-matched':'badge-closed')+'">'+(d.status==='open'?'모집중':d.status==='matched'?'운행중':'마감')+'</span>'+
          '</div>'+
          ...
          (d.status==='matched'?
          '<button onclick="_yShowMatchedDrivers(\\''+d.id+'\\',\\''+d.agencyId+'\\')" style="padding:10px;background:var(--gnl);border:none;border-radius:10px;color:var(--gn);font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;grid-column:span 2"> 기사 현황 + 정산</button>'+
          '<button onclick="_showApplicants(\\''+d.id+'\\')" style="padding:10px;background:var(--brl);border:none;border-radius:10px;color:var(--br);font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">👥 지원자</button>':
          ...
          '<button onclick="_togglePost(\\''+d.id+'\\',\\''+d.status+'\\')" style="padding:10px;background:var(--bg3);border:none;border-radius:10px;color:var(--t2);font-size:13px;cursor:pointer;font-family:inherit">'+(d.status==='open'?'마감처리':d.status==='matched'?'완료처리':'재오픈')+'</button>'+
...
function _yShowMatchedDrivers(postId,agencyId){
  var body=document.getElementById('modal-body');
  body.innerHTML='<div style="font-size:18px;font-weight:900;margin-bottom:16px"> 기사 현황 + 정산</div>'+
  '<div id="matched-driver-list">'+_skRows(2)+'</div>'+
  '<div id="matched-weekly"></div>';
  _openModal();
  _db.collection('yongcha_applies').where('postId','==',postId).where('status','==','approved').get()
  .then(function(snap){
    var list=document.getElementById('matched-driver-list');if(!list)return;
    if(snap.empty){list.innerHTML='<div class="empty"><div class="empty-ico">📭</div><div class="empty-msg">배차 확정된 기사가 없어요</div></div>';return;}
    list.innerHTML='<div style="font-size:13px;font-weight:800;color:var(--t2);margin-bottom:8px">운행중 기사</div>';
    snap.forEach(function(doc){
      var a=doc.data();
      var card=document.createElement('div');card.className='card';card.style.marginBottom='8px';
      card.innerHTML=
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">'+
        '<div style="font-weight:800">🚗 '+_esc(a.driverName)+'</div>'+
        '<span style="font-size:11px;color:var(--t2)">📞 '+(a.driverPhone||'—')+'</span></div>'+
        (a.confirmedStartDate?'<div style="font-size:11px;color:var(--gn);margin-bottom:6px">▶ 시작: '+a.confirmedStartDate+'</div>':'')+
        '<button onclick="_openChatRoom(\\''+a.driverId+'\\',\\''+_jsq(a.driverName)+'\\',\\'driver\\')" style="width:100%;padding:8px;background:var(--bg3);border:none;border-radius:8px;color:var(--t2);font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">💬 채팅하기</button>';
      list.appendChild(card);
    });
    // 주간 정산 섹션
    var weeklyEl=document.getElementById('matched-weekly');
...
/* 상태 전환: open → closed(마감), matched → closed(완료), closed → open(재오픈)
...
function _togglePost(id,status){
  ...
  _db.collection('yongcha_posts').doc(id).update({status:next}).then(function(){
    _yToast(next==='open'?'공고 재오픈됐어요':wasMatched?'운행 완료 처리됐어요':'공고 마감됐어요');
    if(wasMatched)_yCompleteRoutes(id);
    _pgMyPosts(document.getElementById('content'));
  }).catch(function(e){_yToast('오류: '+e.message);});
}

/* 노선 완료 → 배차 확정된 기사들의 completedRoutes +1 (신뢰등급 반영)
...
function _showApplicants(postId){
  // 스켈레톤을 먼저 띄우고 데이터가 오면 교체 (체감 지연 제거)
  var mb0=document.getElementById('modal-body');
  mb0.innerHTML='<div style="font-size:20px;font-weight:900;letter-spacing:-.6px;margin-bottom:16px">👥 지원자 목록</div>'+_skRows(3);
  _openModal();

  _db.collection('yongcha_applies').where('postId','==',postId).get().then(function(snap){
    var body=document.getElementById('modal-body');
    body.innerHTML='<div style="font-size:20px;font-weight:900;letter-spacing:-.6px;margin-bottom:16px">👥 지원자 목록 ('+snap.size+'명)</div>';
    if(snap.empty){
      body.innerHTML+=_emptyHtml('📭','아직 지원자가 없어요','공고가 노출되면 지원이 들어와요')+
    ...
    var profileFetches=applyDocs.map(function(doc){
      return _db.collection('yongcha_users').doc(doc.data().driverId).get()
        .then(function(pd){return {a:Object.assign({id:doc.id},doc.data()),p:pd.exists?pd.data():{}};})
        .catch(function(){return {a:Object.assign({id:doc.id},doc.data()),p:{}};});
    });
    Promise.all(profileFetches).then(function(items){
      items.forEach(function(item){
        var a=item.a, p=item.p;
        var grade=_driverGrade(p.trustScore||0, p.completedRoutes||0);
        var card=document.createElement('div');card.className='applicant-card';
        card.innerHTML=
          ...
          '<button class="judge-btn judge-reject" onclick="_judgeApply(\\''+a.id+'\\',\\'rejected\\',\\''+_jsq(a.driverName)+'\\',\\''+a.driverId+'\\')">거절</button>'+
          '</div>':
          a.status==='approved'?
    ...
    if(driverId){
      if(status==='approved'){
        _db.collection('yongcha_applies').doc(applyId).get().then(function(ap){
          if(!ap.exists)return;
          var a=ap.data();
          var postTitle=a.area||a.courier||'노선';
          ...
          _db.collection('yongcha_contracts').add({
            ...
            driverId:driverId,driverName:name,
            status:'pending_sign',agencySigned:false,driverSigned:false,
...
function _yAutoChat(driverId,driverName,applyData,postTitle,unitPrice,startDate){
  var ids=[_CU.uid,driverId].sort();
  var chatId=ids[0]+'__'+ids[1];
  var chatRef=_db.collection('yongcha_chats').doc(chatId);
  var names={};names[_CU.uid]=_CU.name;names[driverId]=driverName;
  ...
  chatRef.set({
    ...
    lastMessage:firstMsg,lastAt:firebase.firestore.FieldValue.serverTimestamp(),
...
function _doConfirmStart(applyId,postId){
  var d=(document.getElementById('confirm-start-date')||{}).value;
  if(!d){_yToast('날짜를 선택해주세요');return;}
  var batch=_db.batch();
  batch.update(_db.collection('yongcha_applies').doc(applyId),{
    startConfirmed:true,confirmedStartDate:d
  });
  if(postId){
    batch.update(_db.collection('yongcha_posts').doc(postId),{
      status:'matched',matchedDriverId:_CU.uid,matchedAt:firebase.firestore.FieldValue.serverTimestamp()
    });
  }
...
function _yInputDaily(applyId, unitPrice){
  var today=new Date().toISOString().split('T')[0];
  var body=document.getElementById('modal-body');
  body.innerHTML=
  '<div style="font-size:18px;font-weight:900;margin-bottom:4px">📦 오늘 건수 입력</div>'+
  '<div style="font-size:12px;color:var(--t2);margin-bottom:16px">'+today+'</div>'+
  '<div class="inp-wrap"><label class="inp-lbl">배송 건수</label>'+
  '<input class="inp" id="daily-count" type="number" min="0" placeholder="예: 120" oninput="_yCalcDailyEst('+unitPrice+')"></div>'+
  '<div id="daily-est" style="display:none;padding:12px;background:var(--gnl);border-radius:10px;margin-bottom:16px;text-align:center">'+
  '<div style="font-size:12px;color:var(--t2)">오늘 예상 수입</div>'+
  '<div style="font-size:24px;font-weight:900;color:var(--gn)" id="daily-est-val">0원</div></div>'+
  '<div class="inp-wrap"><label class="inp-lbl">메모 (선택)</label>'+
  '<input class="inp" id="daily-memo" placeholder="특이사항 입력..."></div>'+
  '<button onclick="_ySaveDailyRecord(\\''+applyId+'\\',\\''+today+'\\','+unitPrice+')" style="width:100%;padding:15px;background:linear-gradient(135deg,var(--gn),#16a34a);color:#fff;border:none;border-radius:var(--r);font-size:15px;font-weight:800;cursor:pointer;font-family:inherit">저장</button>';
  _openModal();
}

function _yCalcDailyEst(unitPrice){
  var cnt=parseInt((document.getElementById('daily-count')||{}).value)||0;
  var est=cnt*unitPrice;
  var estEl=document.getElementById('daily-est');
  var valEl=document.getElementById('daily-est-val');
  if(estEl)estEl.style.display=cnt>0?'block':'none';
  if(valEl)valEl.textContent=est.toLocaleString()+'원';
}

function _ySaveDailyRecord(applyId,date,unitPrice){
  var cnt=parseInt((document.getElementById('daily-count')||{}).value)||0;
  if(!cnt){_yToast('건수를 입력해주세요');return;}
  var memo=(document.getElementById('daily-memo')||{}).value||'';
  // Get apply data for context
  _db.collection('yongcha_applies').doc(applyId).get().then(function(ap){
    if(!ap.exists)return;
    var a=ap.data();
    ...
    _db.collection('yongcha_daily_records').doc(recordId).set({
      ...
      date:date,count:cnt,unitPrice:unitPrice,
      amount:cnt*unitPrice,memo:memo,status:'pending',
      createdAt:firebase.firestore.FieldValue.serverTimestamp()
    }).then(function(){
...
function _yShowWeeklyDriver(el){
  ...
  .get().then(function(snap){
    var list=document.getElementById('weekly-driver-list');if(!list)return;
    if(snap.empty){list.innerHTML='<div style="font-size:12px;color:var(--t2);text-align:center;padding:12px">이번 주 입력된 건수가 없어요</div>';return;}
    var totalCnt=0,totalAmt=0;
    var rows='';
    var _rDocs=snap.docs.sort(function(a,b){return a.data().date<b.data().date?-1:1;});
    _rDocs.forEach(function(doc){
      var r=doc.data();
      totalCnt+=r.count||0;totalAmt+=r.amount||0;
      rows+='<div style="display:flex;justify-content:space-between;font-size:12px;padding:6px 0;border-bottom:1px solid var(--bd)">'+
        '<span>'+r.date+'</span><span>'+r.count+'건</span>'+
        '<span style="font-weight:700;color:var(--gn)">'+Number(r.amount).toLocaleString()+'원</span>'+
        '<span style="font-size:10px;color:'+(r.status==='confirmed'?'var(--gn)':'var(--t3)')+'">'+
        (r.status==='confirmed'?'확인됨':'대기')+'</span></div>';
    });
    list.innerHTML=
      '<div style="background:var(--gnl);border-radius:10px;padding:12px;margin-bottom:10px;text-align:center">'+
      '<div style="font-size:12px;color:var(--t2)">주간 합계</div>'+
...
function _signContract(contractId, role){
  ...
  _db.collection('yongcha_contracts').doc(contractId).update(upd).then(function(){
    ...
    _db.collection('yongcha_contracts').doc(contractId).get().then(function(snap){
      ...
      var c=snap.data();
      if(c.agencySigned&&c.driverSigned){
...
function _pgPostWrite(el){
  ...
  '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">'+
    ...
    '</div>'+
  '</div>'+
  '<input class="inp" id="pw-price" type="number" placeholder="예: 880" oninput="_calcEst();_loadRegionStats()">'+
  '</div>'+
...
function _yOpenCalc(prefill){
  ...
  body.innerHTML=
    ...
    '<div class="form-section" style="margin-bottom:18px">'+
    '<div class="form-section-title">월 고정 지출</div>'+
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">'+
      '<div class="inp-wrap" style="margin-bottom:0"><label class="inp-lbl">유류비</label>'+
...
function _admSwitch(tab){
  ...
  if(tab==='members'&&btns[0])btns[0].classList.add('on');
  if(tab==='posts'&&btns[1])btns[1].classList.add('on');
  if(tab==='cleanup'&&btns[2])btns[2].classList.add('on');
  if(tab==='members')_admLoadMembers();
  else if(tab==='posts')_admLoadPosts();
...
function _admLoadPosts(){
  var el=document.getElementById('adm-content');if(!el)return;
  el.innerHTML=_skRows(3);
  _db.collection('yongcha_posts').orderBy('createdAt','desc').limit(60).get().then(function(snap){
    if(!el)return;
    if(snap.empty){el.innerHTML='<div class="empty"><div class="empty-ico">📭</div><div class="empty-msg">공고가 없어요</div></div>';return;}
    el.innerHTML='';
    snap.forEach(function(doc){
      ...
      var card=document.createElement('div');card.className='card';
      if(isPremium)card.style.borderLeft='3px solid var(--pu)';
      card.innerHTML=
        '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">'+
        '<div style="flex:1;min-width:0">'+
        (isPremium?'<div style="font-size:10px;color:var(--pu);font-weight:700;margin-bottom:4px">PREMIUM</div>':'')+
        '<div style="font-weight:800">'+_esc(d.area)+'</div>'+
        '<div style="font-size:11px;color:var(--t2);margin-top:3px">🚚 '+(d.courier||'')+' · 📍 '+(d.region||'')+'</div>'+
        '<div style="font-size:11px;color:var(--t3);margin-top:2px">'+(d.agencyName||'')+'</div>'+
        '</div>'+
        '<div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px">'+
        '<span class="status-badge '+(isOpen?'badge-open':'badge-closed')+'">'+(isOpen?'모집중':'마감')+'</span>'+
        '<button onclick="_togglePremium(\\''+d.id+'\\','+isPremium+')" style="padding:5px 10px;background:rgba(109,40,217,.12);color:var(--pu);border:none;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">'+
          (isPremium?'프리미엄 해제':'프리미엄')+'</button>'+
        (isOpen?'<button onclick="_forceClosePost(\\''+d.id+'\\')" style="padding:5px 10px;background:var(--rdl);color:var(--rd);border:none;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">강제 마감</button>':'')+
        '</div>'+
        '</div>';
      el.appendChild(card);
    });
...
function _admLoadCleanup(){
  ...
  '</div>'+
  '<div class="card" style="border-left:3px solid var(--or,#ff6b00)">'+
    '<div style="font-weight:800;margin-bottom:6px"> Playwright 테스트 공고 삭제</div>'+
    '<div style="font-size:12px;color:var(--t2);margin-bottom:12px">자동화 테스트로 생성된 공고를 삭제합니다</div>'+
    '<div id="cleanup-pw-info" style="font-size:12px;color:var(--t3);margin-bottom:10px">공고 수 확인 중...</div>'+
    '<button onclick="_yCleanPlaywrightPosts()" style="padding:10px 16px;background:#fff3e0;color:#e65100;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">🗑 Playwright 공고 삭제</button>'+
  '</div>'+
  '<div class="card" style="border-left:3px solid var(--pu)">'+
    '<div style="font-weight:800;margin-bottom:6px">🔁 중복 공고 정리</div>'+
    '<div style="font-size:12px;color:var(--t2);margin-bottom:12px">같은 제목+대리점의 공고 중 최신 1개만 남깁니다</div>'+
...
/* 예전 구현은 area 문자열에서만 'Playwright' 를 찾았다.
      실제 테스트 데이터는 area 가 '해운대구 좌동 일대' 같은 정상값이라 한 건도 못 잡았고,
      그래서 16건이 계속 목록에 남아 있었다.
      → 소유 계정(@ytest.io / 이름에 테스트·Playwright)으로 판별하고,
        routeNo 의 PW 마커와 제목 마커까지 함께 본다. */
function _yTestOwnerIds(){
  return _db.collection('yongcha_users').limit(500).get().then(function(snap){
    var ids={};
...
function _yScanTestDocs(){
  return _yTestOwnerIds().then(function(ids){
    return Promise.all([
      _db.collection('yongcha_posts').limit(500).get(),
      _db.collection('yongcha_jobs').limit(500).get()
    ]).then(function(r){
      var posts=r[0].docs.filter(function(d){return _yIsTestDoc(d.data(),ids);}).map(function(d){return d.id;});
      var jobs =r[1].docs.filter(function(d){return _yIsTestDoc(d.data(),ids);}).map(function(d){return d.id;});
      return {posts:posts,jobs:jobs,accounts:Object.keys(ids).length};
    });
  });
}

function _yCountPlaywrightPosts(){
...
function _yCleanPlaywrightPosts(){
  _yScanTestDocs().then(function(r){
    ...
    return batch.commit().then(function(){
      _yToast('테스트 데이터 '+Math.min(total,450)+'건 삭제 완료');
      _yCountPlaywrightPosts();
    });
  }).catch(function(e){_yToast('오류: '+e.message);});
}

/* 예전 구현은 (구역+대리점)만 비교해서, 같은 구역의 서로 다른 노선 공고까지
      중복으로 지워버렸다. 지금은 _yPostKey / _yJobKey 지문(노선번호·단가·시작일 포함)으로
      실제로 동일한 것만 대상으로 삼는다. */
function _yScanDup(coll,keyFn){
  return _db.collection(coll).get().then(function(snap){
    var docs=snap.docs.map(function(d){return Object.assign({id:d.id,_ts:(d.data().createdAt&&d.data().createdAt.seconds)||0},d.data());})
      .sort(function(a,b){return b._ts-a._ts;});   // 최신 우선 → 오래된 중복을 지운다
    var seen={},toDelete=[];
    docs.forEach(function(d){
      var k=keyFn(d);
      if(seen[k])toDelete.push(d.id); else seen[k]=true;
    });
    return toDelete;
  });
}

function _yCountDupPosts(){
  var el=document.getElementById('cleanup-dup-info');
...
// ── 구인구직 탭 ──────────────────────────────────────────────
var _jSubTab='list'; // 'list'|'resume' for driver, 'myjobs'|'resumes' for agency

function _pgJobs(el){
  if(_CU.type==='driver') _pgJobsDriver(el);
  else _pgJobsAgency(el);
}
...
function _jSwitchAgency(tab){
  ...
  document.querySelectorAll('.sub-tab').forEach(function(b){b.classList.remove('on');});
  var btns=document.querySelectorAll('.sub-tab');
  if(tab==='myjobs'&&btns[0])btns[0].classList.add('on');
  if(tab==='resumes'&&btns[1])btns[1].classList.add('on');
  var el=document.getElementById('jobs-content');
  if(!el)return;
  if(tab==='myjobs') _loadMyJobs(el);
  else _loadResumes(el);
}
  ...
  q.get().then(function(snap){
    ...
    _jdd2.items.forEach(function(d){
      ...
      card.innerHTML=
        ...
        '<div style="font-size:12px;color:var(--t2);margin-bottom:10px">📍 '+(d.region||'')+(d.area?' · '+d.area:'')+
          ' · 💰 월 '+Number(d.salary||0).toLocaleString()+'원</div>'+
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">'+
        '<button onclick="event.stopPropagation();_toggleJobStatus(\\''+d.id+'\\',\\''+d.status+'\\')" style="padding:9px;background:var(--bg3);border:none;border-radius:10px;color:var(--t2);font-size:12px;cursor:pointer;font-family:inherit">'+(d.status==='open'?'마감 처리':'재오픈')+'</button>'+
        '<button onclick="event.stopPropagation();_loadJobApplicants(\\''+d.id+'\\')" style="padding:9px;background:var(--pul);border:none;border-radius:10px;color:var(--pu);font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">👥 지원자 확인</button>'+
        '</div>';
      list.appendChild(card);
    });
  }).catch(function(e){
...
function _loadResumes(el){
  ...
  .then(function(snap){
    ...
    list.innerHTML='';
    var _resDocs=snap.docs.sort(function(a,b){var at=a.data().updatedAt;var bt=b.data().updatedAt;return (bt&&bt.seconds||0)-(at&&at.seconds||0);}).slice(0,50);
    _resDocs.forEach(function(doc){
      var r=Object.assign({id:doc.id},doc.data());
      var card=document.createElement('div');card.className='resume-card';
      card.innerHTML=
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">'+
        '<div style="font-size:15.5px;font-weight:800">🚗 '+_esc(r.driverName)+'</div>'+
        ...
        '<button onclick="_scoutDriver(\\''+r.driverId+'\\',\\''+_jsq(r.driverName)+'\\',\\''+_jsq(r.driverPhone)+'\\')" style="width:100%;padding:10px;background:var(--brl);color:var(--br);border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">📞 스카웃 연락하기</button>';
      list.appendChild(card);
    });
  }).catch(function(e){
...
function _scoutDriver(driverId, driverName, driverPhone){
  var body=document.getElementById('modal-body');
  body.innerHTML=
  '<div style="font-size:20px;font-weight:900;margin-bottom:16px">📞 스카웃 연락</div>'+
  '<div class="card" style="margin-bottom:16px">'+
  '<div style="font-size:15.5px;font-weight:800;margin-bottom:8px">🚗 '+_esc(driverName)+' 기사님</div>'+
  '<div style="font-size:13.5px;color:var(--t2)">📞 '+_esc(driverPhone)+'</div>'+
  '</div>'+
...
function _replyScout(scoutId, accept){
  _db.collection('yongcha_scouts').doc(scoutId).update({
    status:accept?'accepted':'rejected',
    repliedAt:firebase.firestore.FieldValue.serverTimestamp()
  }).then(function(){
    _yToast(accept?'스카웃 제안을 수락했어요!':'스카웃 제안을 거절했어요');
    _pgMyApplies(document.getElementById('content'));
  }).catch(function(e){_yToast('오류: '+e.message);});
}
...
function _loadJobApplicants(jobId){
  _db.collection('yongcha_applies').where('jobId','==',jobId).get().then(function(snap){
    var body=document.getElementById('modal-body');
    body.innerHTML='<div style="font-size:18px;font-weight:900;margin-bottom:16px">👥 배차 지원자</div>';
    ...
    snap.forEach(function(doc){
      ...
      card.innerHTML=
        ...
        '<span style="font-size:12px;font-weight:700;color:'+(statusColor[a.status]||'var(--t2)')+'">'+
        (statusMap[a.status]||a.status)+'</span></div>'+
        '<div class="applicant-meta">📍 '+(a.driverRegion||'')+'</div>'+
        (a.status==='pending'?
        '<div class="judge-row">'+
        '<button class="judge-btn judge-approve" onclick="_judgeJobApply(\\''+a.id+'\\',\\'approved\\',\\''+_jsq(a.driverName)+'\\',\\''+a.driverId+'\\')">승인</button>'+
        '<button class="judge-btn judge-reject" onclick="_judgeJobApply(\\''+a.id+'\\',\\'rejected\\',\\''+_jsq(a.driverName)+'\\',\\''+a.driverId+'\\')">미선발</button>'+
        '</div>':'');
      body.appendChild(card);
    });
    _openModal();
...
function _pgJobsDriver(el){
  ...
  _jSwitchDriver(activeTab);
}

function _jSwitchDriver(tab){
  _jSubTab=tab;
  document.querySelectorAll('.sub-tab').forEach(function(b){b.classList.remove('on');});
  var btns=document.querySelectorAll('.sub-tab');
  if(tab==='list'&&btns[0])btns[0].classList.add('on');
  if(tab==='resume'&&btns[1])btns[1].classList.add('on');
  var el=document.getElementById('jobs-content');
  if(!el)return;
  if(tab==='list') _loadJobList(el);
  else _loadMyResume(el);
}
  ...
  .then(function(snap){
    ...
    _jlDocs.forEach(function(d){
      ...
      card.innerHTML=
        ...
        '🏢 '+_esc(d.agencyName)+' · 📍 '+_esc(d.region)+(d.area?' '+_esc(d.area):'')+
        '</div>'+
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px">'+
        '<div style="background:var(--bg3);border-radius:10px;padding:8px 10px">'+
        '<div style="font-size:10px;color:var(--t2);margin-bottom:2px">월 급여</div>'+
        '<div style="font-size:15px;font-weight:900;color:var(--br)">'+Number(d.salary||0).toLocaleString()+'<span style="font-size:10px;color:var(--t2)">원</span></div>'+
        '</div>'+
        '<div style="background:var(--bg3);border-radius:10px;padding:8px 10px">'+
        '<div style="font-size:10px;color:var(--t2);margin-bottom:2px">경력</div>'+
        '<div style="font-size:14px;font-weight:700">'+_careerLabel(d.careerReq)+'</div>'+
        '</div>'+
        '</div>'+
...
function _showJobDetail(d){
  ...
  (d.extras?'<div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:14px">'+
  d.extras.split(',').filter(Boolean).map(function(e){
    return '<span style="font-size:11px;background:var(--bg3);padding:3px 9px;border-radius:8px;color:var(--t2)">'+_esc(e)+'</span>';
  }).join('')+'</div>':'')+
  '<button id="job-apply-btn" onclick="_applyJob(\\''+d.id+'\\',\\''+d.agencyId+'\\',\\''+_jsq(d.agencyName)+'\\')" style="width:100%;padding:16px;background:linear-gradient(135deg,var(--br),var(--br2));color:#000;border:none;border-radius:var(--r);font-size:16px;font-weight:800;cursor:pointer;font-family:inherit">🙋 지원하기</button>';
  _openModal();

  // 이미 지원 여부
  _db.collection('yongcha_applies').where('jobId','==',d.id).where('driverId','==',_CU.uid).get().then(function(snap){
...
// ── 내 이력서 ──
function _loadMyResume(el){
  el.innerHTML='<div id="resume-area">'+_skRows(3)+'</div>';
  _db.collection('yongcha_resumes').where('driverId','==',_CU.uid).limit(1).get()
  .then(function(snap){
    var area=document.getElementById('resume-area');if(!area)return;
    if(snap.empty){
      _renderResumeForm(area,null,null);
    } else {
      var doc=snap.docs[0];
...
function _renderResumeForm(el, docId, data){
  var d=data||{};
  el.innerHTML=
  '<div style="margin-bottom:14px;display:flex;align-items:center;justify-content:space-between">'+
  '<div style="font-size:15px;font-weight:800">📄 내 이력서</div>'+
  (docId?'<span style="font-size:11px;padding:3px 9px;border-radius:20px;background:'+(d.isPublic?'var(--gnl)':'var(--bg3)')+';color:'+(d.isPublic?'var(--gn)':'var(--t3)')+';">'+(d.isPublic?'공개중':'비공개')+'</span>':'')+
  '</div>'+

  '<div class="form-section">'+
  '<div class="form-section-title">경력</div>'+
  '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px" id="rc-career-group">'+
  ['신입','1년 미만','1~3년','3~5년','5년 이상'].map(function(c){
    return '<button onclick="_selType(this,\\''+c+'\\',\\'rc-career\\')" style="padding:9px;border-radius:10px;border:1.5px solid '+(d.career===c?'var(--ac)':'var(--border)')+';background:'+(d.career===c?'var(--acl)':'transparent')+';color:'+(d.career===c?'var(--ac)':'var(--t2)')+';font-size:12px;font-weight:700;cursor:pointer">'+c+'</button>';
  }).join('')+
  '</div><input type="hidden" id="rc-career" value="'+(d.career||'')+'">'+
  '<div class="inp-wrap"><label class="inp-lbl">경력 상세 (선택)</label>'+
  '<textarea class="inp" id="rc-careerDesc" rows="2" placeholder="담당 노선, 택배사, 주요 업무 등" style="resize:none">'+(d.careerDesc||'')+'</textarea></div>'+
  '</div>'+

  '<div class="form-section">'+
  '<div class="form-section-title">차량 정보</div>'+
  '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px" id="rc-vehicle-group">'+
  CAR_TYPES.concat(['무관']).map(function(v){
    return '<button onclick="_selType(this,\\''+v+'\\',\\'rc-vehicle\\')" style="padding:9px;border-radius:10px;border:1.5px solid '+(d.vehicleType===v?'var(--ac)':'var(--border)')+';background:'+(d.vehicleType===v?'var(--acl)':'transparent')+';color:'+(d.vehicleType===v?'var(--ac)':'var(--t2)')+';font-size:12px;font-weight:700;cursor:pointer">'+v+'</button>';
  }).join('')+
  '</div><input type="hidden" id="rc-vehicle" value="'+(d.vehicleType||'')+'">'+
  '</div>'+

  '<div class="form-section">'+
  '<div class="form-section-title">희망 조건</div>'+
  '<div class="inp-wrap"><label class="inp-lbl">희망 지역</label>'+
  '<select class="inp" id="rc-region">'+
  ['부산','대구','서울','경기','인천','광주','대전','울산','경남','경북','전남','전북','충남','충북','강원','제주'].map(function(r){
    return '<option value="'+r+'"'+(d.preferRegion===r?' selected':'')+'>'+r+'</option>';
  }).join('')+
  '</select></div>'+
  '<div class="inp-wrap"><label class="inp-lbl">희망 월 급여 (원)</label>'+
  '<input class="inp" id="rc-pay" type="number" placeholder="예: 3500000" value="'+(d.preferPay||'')+'"></div>'+
  '</div>'+

  '<div class="form-section">'+
  '<div class="form-section-title">자기소개</div>'+
  '<div class="inp-wrap">'+
  '<textarea class="inp" id="rc-intro" rows="4" placeholder="간단한 자기소개, 장점, 지원 동기 등을 적어주세요" style="resize:none">'+(d.selfIntro||'')+'</textarea></div>'+
  '</div>'+

  '<div style="display:flex;align-items:center;justify-content:space-between;padding:14px;background:var(--bg3);border-radius:var(--r);margin-bottom:16px">'+
  '<div><div style="font-size:14px;font-weight:700">이력서 공개</div>'+
  '<div style="font-size:11px;color:var(--t2);margin-top:2px">대리점이 내 이력서를 볼 수 있어요</div></div>'+
  '<button class="toggle '+(d.isPublic?'on':'')+'" id="rc-public-toggle" onclick="this.classList.toggle(\\'on\\')"></button>'+
  '</div>'+

  '<button class="btn-main" id="rc-submit-btn" onclick="_submitResume(\\''+docId+'\\')">'+
  (docId?'💾 이력서 수정하기':'📄 이력서 등록하기')+'</button>';
}

function _submitResume(docId){
  var career=(document.getElementById('rc-career')||{}).value||'';
  var careerDesc=(document.getElementById('rc-careerDesc')||{}).value||'';
  var vehicle=(document.getElementById('rc-vehicle')||{}).value||'';
  var region=(document.getElementById('rc-region')||{}).value||_CU.region||'';
  var pay=parseInt((document.getElementById('rc-pay')||{}).value)||0;
  var intro=(document.getElementById('rc-intro')||{}).value||'';
  var isPublic=document.getElementById('rc-public-toggle')&&document.getElementById('rc-public-toggle').classList.contains('on');
  var btn=document.getElementById('rc-submit-btn');

  if(!career){_yToast('경력을 선택해주세요');return;}
  btn.textContent='저장 중...';btn.disabled=true;

  var data={
    driverId:_CU.uid, driverName:_CU.name,
    driverPhone:_CU.phone, driverRegion:_CU.region,
    career:career, careerDesc:careerDesc,
    vehicleType:vehicle, preferRegion:region,
    preferPay:pay, selfIntro:intro, isPublic:isPublic,
    updatedAt:firebase.firestore.FieldValue.serverTimestamp()
  };

  var p=docId
    ? _db.collection('yongcha_resumes').doc(docId).update(data)
    : _db.collection('yongcha_resumes').add(Object.assign(data,{createdAt:firebase.firestore.FieldValue.serverTimestamp()}));

  p.then(function(){
    _yToast(docId?'이력서가 수정됐어요!':'이력서가 등록됐어요!');
    _jSubTab='resume';
    _pgJobsDriver(document.getElementById('content'));
  }).catch(function(e){
    btn.textContent=docId?'💾 이력서 수정하기':'📄 이력서 등록하기';btn.disabled=false;
    _yToast('오류: '+e.message);
  });
}
...
function _showJobWriteModal(){
  ...
  '<select class="inp" id="jw-courier"><option value="">선택</option>'+
  ['CJ대한통운','한진택배','롯데택배','우체국','로젠택배','쿠팡로지스틱스'].map(function(c){return '<option>'+c+'</option>';}).join('')+
  '</select></div>'+

  '<div class="inp-wrap"><label class="inp-lbl">구역 / 담당 지역</label>'+
  '<input class="inp" id="jw-area" placeholder="예: 해운대구 좌동 일대"></div>'+

  '<div class="inp-wrap"><label class="inp-lbl">차량 요건</label>'+
  '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px" id="jw-vehicle-group">'+
  CAR_TYPES.concat(['무관']).map(function(v){
    return '<button onclick="_selType(this,\\''+v+'\\',\\'jw-vehicle\\')" style="padding:8px;border-radius:10px;border:1.5px solid var(--border);background:transparent;color:var(--t2);font-size:11px;font-weight:700;cursor:pointer">'+v+'</button>';
  }).join('')+
  '</div><input type="hidden" id="jw-vehicle"></div>'+

  '<div class="inp-wrap"><label class="inp-lbl">경력 요건</label>'+
  '<div style="display:flex;gap:6px;flex-wrap:wrap" id="jw-career-group">'+
  ['신입','1년 이상','3년 이상','무관'].map(function(c){
    return '<button onclick="_selType(this,\\''+c+'\\',\\'jw-career\\')" style="padding:8px 14px;border-radius:20px;border:1.5px solid var(--border);background:transparent;color:var(--t2);font-size:12px;font-weight:700;cursor:pointer">'+c+'</button>';
  }).join('')+
  '</div><input type="hidden" id="jw-career"></div>'+

  '<div class="inp-wrap"><label class="inp-lbl">월 급여 (원) <span style="color:var(--rd)">*</span></label>'+
  '<input class="inp" id="jw-salary" type="number" placeholder="예: 3500000"></div>'+

  '<div class="inp-wrap"><label class="inp-lbl">정산일</label>'+
  '<input class="inp" id="jw-settle" placeholder="예: 매월 25일"></div>'+
...
function _submitJob(){
  ...
  var payload={
    agencyId:_CU.uid, agencyName:_CU.name, agencyRating:_CU.rating||0,
    region:_CU.region, area:get('jw-area'), courier:get('jw-courier'),
    jobTitle:title, jobType:jobType,
    vehicleType:get('jw-vehicle'), careerReq:get('jw-career'),
    salary:parseInt(salary), settleDay:get('jw-settle'),
    workDays:get('jw-days'), headcount:parseInt(get('jw-headcount'))||1,
    openEnded:openEnded, desc:get('jw-desc'),
    status:'open', createdAt:firebase.firestore.FieldValue.serverTimestamp()
  };
  var myKey=_yJobKey(payload);
  ...
  .then(function(snap){
    ...
    btn.textContent='등록 중...';
    return _db.collection('yongcha_jobs').add(payload);
  })
  .then(function(){
    _unlockJ();
    _yToast('채용공고가 등록됐어요!');
...
function _careerLabel(c){
  var map={'신입':'신입','1년 미만':'1년 미만','1~3년':'1~3년','3~5년':'3~5년','5년 이상':'5년 이상',
    '1년 이상':'1년 이상','3년 이상':'3년 이상','무관':'무관'};
  return map[c]||c||'무관';
}
...
/* 채팅 목록 — 이전엔 get() 1회라 새 메시지가 와도 목록이 갱신되지 않았다.
   onSnapshot 으로 바꿔 실시간 반영한다. */
var _chatListUnsub=null;
function _pgChats(el){
  el.innerHTML=
    '<div class="page-hdr"><h1 class="page-title">채팅</h1>'+
    '<p class="page-sub">대리점·기사와 직접 소통해요</p></div>'+
    '<div id="chat-list">'+_skRows(3)+'</div>';
  ...
  _chatListUnsub=_db.collection('yongcha_chats').where('participants','array-contains',_CU.uid)
    ...
    .onSnapshot(function(snap){
      ...
      if(snap.empty){
        list.innerHTML=_emptyHtml('💬','아직 채팅이 없어요','배차가 확정되면 채팅방이 자동으로 열려요');
        return;
      }
      ...
      docs.forEach(function(doc){
        var d=Object.assign({id:doc.id},doc.data());
        var otherUid=(d.participants||[]).filter(function(u){return u!==_CU.uid;})[0]||'';
        var otherName=(d.participantNames||{})[otherUid]||'상대방';
        var otherType=(d.participantTypes||{})[otherUid]||'';
        var unread=(d.unread||{})[_CU.uid]||0;
        var lastAt='';
        if(d.lastAt&&d.lastAt.seconds){
          var dt=new Date(d.lastAt.seconds*1000);
          if(new Date().toDateString()===dt.toDateString())
            lastAt=('0'+dt.getHours()).slice(-2)+':'+('0'+dt.getMinutes()).slice(-2);
          else lastAt=(dt.getMonth()+1)+'/'+dt.getDate();
        }
        var row=document.createElement('div');
        row.className='chat-row';
        row.onclick=function(){_pgChatRoom(d.id,otherUid,otherName);};
        row.innerHTML=
          '<div class="chat-av">'+(otherType==='agency'?'🏢':otherType==='driver'?'🚗':'👤')+'</div>'+
          '<div style="flex:1;min-width:0">'+
          '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:4px">'+
          '<div style="font-size:14.5px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+_esc(otherName)+'</div>'+
...
function _openChatRoom(otherUid,otherName,otherType){
  ...
  var chatId=ids[0]+'__'+ids[1];
  var chatRef=_db.collection('yongcha_chats').doc(chatId);
  chatRef.get().catch(function(){return {exists:false};}).then(function(snap){
    if(!snap||!snap.exists){
      var names={};names[_CU.uid]=_CU.name;names[otherUid]=otherName;
      var types={};types[_CU.uid]=_CU.type;types[otherUid]=otherType;
      var unread={};unread[_CU.uid]=0;unread[otherUid]=0;
      return chatRef.set({
        participants:[_CU.uid,otherUid],
      ...
      items.map(function(a){
        return '<div class="card" style="margin-bottom:10px">'+
          ...
          '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">'+
            ...
            '<button onclick="_showReviewModal(\\''+_esc(a.driverId)+'\\',\\''+_jsq(a.driverName)+'\\',\\'driver\\',\\''+a.id+'\\')" style="min-height:42px;background:'+(a.agencyReviewed?'var(--gnl)':'var(--acl)')+';border:1px solid '+(a.agencyReviewed?'var(--gnln)':'var(--acln)')+';border-radius:10px;color:'+(a.agencyReviewed?'var(--gn)':'var(--ac)')+';font-size:12.5px;font-weight:800;cursor:pointer;font-family:inherit">'+(a.agencyReviewed?'평가완료':'기사 평가')+'</button>'+
          '</div>'+
        '</div>';
      }).join('');
  }).catch(function(e){
...
function _riqRecord(el){
  ...
  if(isDriver){
    html+='<div class="card" style="margin-bottom:14px">'+
      ...
      '<div class="inp-wrap"><label class="inp-lbl">배송 건수</label>'+
      '<input class="inp" id="riq-cnt" type="number" min="0" placeholder="100"></div>'+
      '</div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">'+
      '<div class="inp-wrap"><label class="inp-lbl">단가 (원/건)</label>'+
      '<input class="inp" id="riq-price" type="number" min="0" placeholder="850"></div>'+
      '<div class="inp-wrap"><label class="inp-lbl">택배사</label>'+
      '<select class="inp" id="riq-courier">'+
      '<option value="">선택</option>'+
      '<option>CJ대한통운</option><option>롯데택배</option><option>한진택배</option>'+
      '<option>쿠팡로지스틱스</option><option>컬리로지스틱스</option>'+
      '<option>로젠택배</option><option>우체국택배</option><option>기타</option>'+
      '</select></div></div>'+
      '<button class="btn-main" id="riq-save-btn" onclick="_riqSaveRecord()">실적 저장 및 공유</button>'+
      '</div>';
  }
...
function _yCompleteWork(applyId){
  var body=document.getElementById('modal-body');
  body.innerHTML=
    '<div style="font-size:19px;font-weight:900;margin-bottom:4px">배송 완료 확정</div>'+
    ...
    '</label>'+
    '<div id="work-photo-preview" style="display:none;margin-bottom:12px">'+
    '<img id="work-photo-img" style="width:100%;border-radius:var(--r-lg);max-height:200px;object-fit:cover">'+
    '</div>'+
...
다음 JSON만 반환하세요:
{
  ...
  "tip": "지원율 높이는 팁 1줄"
    ...
    // ── Gas Stations: 카카오 로컬 API (무료, 월 30만 호출) ──────────────────
      ...
      try {
        ...
        const stations = (data.documents || []).slice(0, 10).map((d, i) => {
          ...
          return {
            ...
            price:   0,          // 카카오는 가격 없음 — 거리순 정렬로 대체
            dist:    parseFloat((distM / 1000).toFixed(2)),
        ...
        // 가격 없으므로 거리 기준 AI 스코어 (가까울수록 높음)
        if (stations.length > 0) {
          const maxD = Math.max(...stations.map(s => s.dist), 0.001);
          stations.forEach(s => {
            s.aiScore = Math.round((1 - s.dist / maxD) * 100);
          });
        }
