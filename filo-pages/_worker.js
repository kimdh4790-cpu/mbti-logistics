...
async function fetchAsset(path, request, env) {
  ...
  if (e && e.DONWAY_ASSETS) {
    const kvBuf = await e.DONWAY_ASSETS.get(fileName, 'arrayBuffer');
    if (kvBuf) {
      return new Response(kvBuf, { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store', 'X-Served-From': 'KV' } });
...
async function serveKVFile(env, fileName, contentType) {
  ...
  try {
    ...
    if (_e && _e.DONWAY_ASSETS) {
      const kvBuf2 = await _e.DONWAY_ASSETS.get(fileName, 'arrayBuffer');
      if (kvBuf2) {
        return new Response(kvBuf2, { headers: { 'Content-Type': contentType+'; charset=utf-8', 'Cache-Control': 'no-store', 'X-Served-From': 'KV', ...SECURITY_HEADERS } });
      }
    }
    // KV 없으면 GitHub Raw
    const PAGES_FILES = {};
