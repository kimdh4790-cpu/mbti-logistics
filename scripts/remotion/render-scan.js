#!/usr/bin/env node
/**
 * Remotion으로 SCAN AI 홍보 영상 렌더링
 * 사용: node scripts/remotion/render-scan.js [--reels] [--dry-run]
 */
...
async function main() {
  ...
  const compositionId = reelsMode ? 'ScanReels' : 'ScanPromo';
  const outFile = reelsMode
    ? path.join(OUT_DIR, 'scan-reels.mp4')
    : path.join(OUT_DIR, 'scan-promo.mp4');

  console.log(`[Remotion] SCAN AI 영상 렌더링 시작 (${compositionId})`);
  ...
  if (fs.existsSync(narFile)) {
    fs.copyFileSync(narFile, path.join(pubDir, 'scan-narration.mp3'));
    hasNarration = true;
  ...
  if (!reelsMode) {
    const finalPath = path.join(OUT_DIR, 'scan-final.mp4');
