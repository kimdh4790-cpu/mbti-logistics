...
async function main() {
  ...
  const compositionId = reelsMode ? 'YongchaDealerReels' : 'YongchaDealerPromo';
  const outFile = reelsMode
    ? path.join(OUT_DIR, 'yongcha-dealer-reels.mp4')
    : path.join(OUT_DIR, 'yongcha-dealer-promo.mp4');

  console.log(`[Remotion] 소장 관점 영상 렌더링 (${compositionId})`);
  ...
  console.log(`\n[Remotion] 완료: ${outFile}`);
  if (!reelsMode) fs.copyFileSync(outFile, path.join(OUT_DIR, 'yongcha-dealer-final.mp4'));
}
