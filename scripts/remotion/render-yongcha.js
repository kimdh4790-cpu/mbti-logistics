#!/usr/bin/env node
/**
 * Remotion으로 용차앱 홍보 영상 렌더링
 * 사용: node scripts/remotion/render-yongcha.js [--reels] [--dry-run]
 */
...
async function main() {
  ...
  const compositionId = reelsMode ? 'YongchaReels' : 'YongchaPromo';
  const outFile = reelsMode
    ? path.join(OUT_DIR, 'yongcha-reels.mp4')
    : path.join(OUT_DIR, 'yongcha-promo.mp4');

  console.log(`[Remotion] 용차앱 영상 렌더링 시작 (${compositionId})`);
  ...
  if (!reelsMode) {
    const finalPath = path.join(OUT_DIR, 'yongcha-final.mp4');
