...
async function record() {
  ...
  const files = fs.readdirSync(outputDir).filter(f => f.endsWith('.webm') && !f.startsWith('yongcha-raw'));
  if (files.length > 0) {
    const newest = files.sort((a, b) =>
      fs.statSync(path.join(outputDir, b)).mtimeMs - fs.statSync(path.join(outputDir, a)).mtimeMs
    )[0];
    fs.renameSync(path.join(outputDir, newest), path.join(outputDir, 'yongcha-raw.webm'));
