...
function _filoPageTableQR(el) {
  ...
  var hdr = document.createElement('div');
  hdr.style.cssText = 'display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px';
  hdr.innerHTML = '<div><div class="page-title">📱 테이블 QR 관리</div>' +
    '<div class="page-sub">테이블별 QR코드를 생성하고 인쇄해요</div></div>';
  wrap.appendChild(hdr);

  // 테이블 수 설정
  var settingCard = document.createElement('div');
  settingCard.className = 'card';
  settingCard.style.marginBottom = '16px';
  settingCard.innerHTML =
    '<div style="font-size:13px;font-weight:800;color:var(--t3);margin-bottom:12px">테이블 설정</div>' +
    '<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">' +
    '<div style="display:flex;align-items:center;gap:8px">' +
    '<label style="font-size:13px;color:var(--t2)">테이블 수</label>' +
    '<input id="tqr-count" type="number" value="10" min="1" max="50" ' +
    'style="width:70px;padding:8px 10px;background:var(--bg3);border:1.5px solid var(--bd);border-radius:8px;color:var(--tx);font-size:14px;font-weight:700">' +
    '</div>' +
    '<div style="display:flex;align-items:center;gap:8px">' +
    '<label style="font-size:13px;color:var(--t2)">매장명</label>' +
    '<input id="tqr-name" type="text" placeholder="매장명 (선택)" ' +
    'style="padding:8px 12px;background:var(--bg3);border:1.5px solid var(--bd);border-radius:8px;color:var(--tx);font-size:13px;width:150px">' +
    '</div>' +
    '<button onclick="_tqrGenerate('' + did + '')" ' +
    'style="padding:9px 18px;background:var(--br);color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer">🔄 QR 생성</button>' +
    '<button onclick="_tqrPrintAll()" ' +
    'style="padding:9px 18px;background:var(--bg3);border:1px solid var(--bd);color:var(--tx);border-radius:10px;font-size:13px;font-weight:700;cursor:pointer">🖨️ 전체 인쇄</button>' +
    '</div>';
  wrap.appendChild(settingCard);
  ...
  // 초기 생성
  setTimeout(function(){ _tqrGenerate(did); }, 300);
}

function _tqrGenerate(did) {
  var count = parseInt((document.getElementById('tqr-count')||{}).value||'10');
  var storeName = (document.getElementById('tqr-name')||{}).value || '';
  var grid = document.getElementById('tqr-grid');
  if(!grid) return;
  grid.innerHTML = '';
  ...
  function doGen() {
    for(var i=1; i<=count; i++) {
      var url = 'https://filo.ai.kr/order?d=' + did + '&t=' + i;
      var card = document.createElement('div');
      card.className = 'card';
      card.style.cssText = 'text-align:center;padding:16px;cursor:pointer';
      card.dataset.url = url;
      card.dataset.table = i;

      var qrDiv = document.createElement('div');
      qrDiv.id = 'qr-t' + i;
      qrDiv.style.cssText = 'width:120px;height:120px;margin:0 auto 10px;background:#fff;border-radius:8px;padding:4px';
      card.appendChild(qrDiv);

      var label = document.createElement('div');
      label.style.cssText = 'font-size:14px;font-weight:800;color:var(--tx);margin-bottom:4px';
      label.textContent = (storeName || '') + ' ' + i + '번 테이블';
      card.appendChild(label);

      var urlLabel = document.createElement('div');
      urlLabel.style.cssText = 'font-size:9px;color:var(--t3);margin-bottom:8px;word-break:break-all';
      urlLabel.textContent = url;
      card.appendChild(urlLabel);

      var printBtn = document.createElement('button');
      printBtn.style.cssText = 'padding:5px 12px;background:var(--br);color:#fff;border:none;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer';
      printBtn.textContent = '🖨️ 인쇄';
      printBtn.onclick = (function(u, t, sn){
        return function(){ _tqrPrint(u, t, sn); };
      })(url, i, storeName);
...
function _tqrPrint(url, tableNum, storeName) {
  var w = window.open('', '_blank', 'width=400,height=500');
  w.document.write('<html><head><title>테이블 QR - ' + tableNum + '번</title>' +
    '<style>body{font-family:sans-serif;text-align:center;padding:20px}' +
    '#qr{margin:0 auto 12px}' +
    'h2{font-size:20px;margin-bottom:4px}' +
    'p{font-size:12px;color:#666}' +
    '@media print{button{display:none}}</style></head><body>' +
    '<h2>' + (storeName||'') + ' ' + tableNum + '번 테이블</h2>' +
    '<p>QR을 스캔해서 주문해주세요</p>' +
    '<div id="qr"></div>' +
    '<p style="font-size:9px;color:#999;margin-top:8px">' + url + '</p>' +
    '<button onclick="window.print()" style="margin-top:12px;padding:10px 24px;background:#f59e0b;color:#000;border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer">🖨️ 인쇄</button>' +
    '<script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"><\/script>' +
    '<script>new QRCode(document.getElementById("qr"),{text:"' + url + '",width:200,height:200,correctLevel:QRCode.CorrectLevel.H});<\/script>' +
    '</body></html>');
  w.document.close();
}

function _tqrPrintAll() {
  var grid = document.getElementById('tqr-grid');
  if(!grid) return;
  var cards = grid.querySelectorAll('.card');
  if(!cards.length) { _filoToast('먼저 QR을 생성해주세요'); return; }
  var storeName = (document.getElementById('tqr-name')||{}).value || '';
  var rows = '';
  cards.forEach(function(card, i) {
    var url = card.dataset.url;
    var t = card.dataset.table;
    rows += '<div style="page-break-inside:avoid;text-align:center;padding:16px;border:1px solid #ddd;border-radius:8px;margin:8px">' +
      '<h3 style="margin:0 0 8px">' + (storeName||'') + ' ' + t + '번 테이블</h3>' +
      '<div id="pqr' + i + '"></div>' +
      '<p style="font-size:9px;color:#999;margin:4px 0">' + url + '</p>' +
      '</div>';
  });
  ...
  w.document.write('<html><head><title>테이블 QR 전체</title>' +
    ...
    '<button onclick="window.print()" style="padding:10px 24px;background:#f59e0b;color:#000;border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer">🖨️ 전체 인쇄</button>' +
    '</div>' +
  ...
  cards.forEach(function(card, i) {
    var url = card.dataset.url;
    w.document.write('new QRCode(document.getElementById("pqr' + i + '"),{text:"' + url + '",width:160,height:160,correctLevel:QRCode.CorrectLevel.H});');
  });
